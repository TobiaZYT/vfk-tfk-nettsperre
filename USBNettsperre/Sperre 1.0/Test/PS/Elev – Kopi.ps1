...
Function Connect-ToVTFK {
    $VTFK_SSID = "VTFK"
    netsh wlan connect name=$VTFK_SSID
}

Function Disconnect-FromVTFK {
    $VTFK_SSID = "V"
    netsh wlan disconnect ssid=$VTFK_SSID
}

$connectedWifi = Get-ConnectedWifi

if ($connectedWifi -ne "VTFK") {
    Write-Output "Kobler til VTFK-nettverket..."
    Connect-ToVTFK
    $connectedWifi = Get-ConnectedWifi
}
...
if ($connectedWifi -eq "VTFK" -and $vg_status) {
    Write-Output "Kobler fra VTFK-nettverket..."
    Disconnect-FromVTFK
    Write-Output "Kobler til VTFK-nettverket på nytt..."
    Connect-ToVTFK
    $connectedWifi = Get-ConnectedWifi
}
...
# Legg til informasjon om forsøk på tilkobling og frakobling av VTFK-nettverket
if ($connectedWifi -eq "VTFK" -and $vg_status) {
    $logContent += "`nDet ble også forsøkt å koble til/fra VTFK uten at det løste noe."
}
